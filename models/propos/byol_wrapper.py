...
from models.propos.groco import GroCo
...
class BYOLWrapper(MoCoWrapper):
    ...
    def forward_loss(self, im_q, im_k, psedo_labels: torch.Tensor):
        ...
        k_1 = self.nn_replacer(k.detach(), update=True)

        contrastive_loss = ((- 2 * F.cosine_similarity(self.predictor(noise_q), k). mean())+\
                           (- 2 * F.cosine_similarity(self.predictor(noise_q), k_1). mean()))


        # groco_ins_loss, _ = self.groco_ins(data=None, output=(noise_q, k))
        ...
        # return contrastive_loss, cluster_loss_batch, groco_ins_loss,all_q, all_k
